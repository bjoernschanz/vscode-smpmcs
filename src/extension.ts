...
export function activate(context: vscode.ExtensionContext): void {
    ...
    diagnosticCollection = vscode.languages.createDiagnosticCollection('smpmcs');

    // start checking the active editor immediately
    if (vscode.window.activeTextEditor?.document.languageId === 'smpmcs')  {
    ...
    context.subscriptions.push(
        vscode.window.onDidChangeActiveTextEditor((editor: vscode.TextEditor | undefined) => {
            // ensure this is a smpmcs document and only check on new activations
            if (editor && editor.document.languageId === 'smpmcs' && !diagnosticCollection.has(editor.document.uri)) {
    ...
    context.subscriptions.push(
        vscode.languages.registerDocumentSymbolProvider(
            {scheme: "file", language: "smpmcs"}, 
            new smpmcsDocumentSymbolProvider()
        )
    );
}
...
function updateDiagnostics(document: vscode.TextDocument): void {
    ...
    let enableDiagnostics = vscode.workspace.getConfiguration().get('smpmcs.diagnostics.enableDiagnostics');
...
class smpmcsDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
